...
- **New "Git" page (frontend).** A `frontend/src/pages/Git.tsx` page with two tabs: **提交记录 (Commits)** — paginated commit list with author, relative time, message, refs, and file-change stats, auto-refreshable; and **推送记录 (Pushes)** — the persisted push history with success/fail status, commit range, message, and time. Add a "Git" nav entry to the Sidebar and a route in `App.tsx`.
- **Dashboard wiring.** The existing "系统发布" button already calls `/api/publish/system`; after a successful push it triggers a refresh of the Git page's data so the new push row appears (no new button).
